...
3. the communicator drops non-audited allreduce backends under invariance
   (source contract): FlashInfer via the upstream guard added in #52998, and
   AITER here. QuickReduce is deliberately NOT asserted -- it is constructed
   only on ROCm and is not excluded under batch invariance today; see the
   note in ``test_communicator_drops_unaudited_backends_under_invariance``.
...
def test_communicator_drops_unaudited_backends_under_invariance():
    ...
    # FlashInfer is excluded upstream, where use_flashinfer_allreduce is
    # computed (#52998). This PR builds on that guard rather than repeating it,
    # so the assertion targets the upstream expression, not our block.
    assert (
        "envs.VLLM_ALLREDUCE_USE_FLASHINFER and not envs.VLLM_BATCH_INVARIANT" in text
    )

    # AITER has no audited fixed reduction order and is excluded by this PR.
    gate = _extract(
        text, "if envs.VLLM_BATCH_INVARIANT:", "self.use_custom_allreduce ="
    )
    assert "use_aiter_allreduce = False" in gate

    # QuickReduce is NOT excluded under batch invariance. It is constructed
    # only when current_platform.is_rocm(), so it is unreachable on the CUDA
    # configurations this PR was validated on, but on ROCm it would be tried
    # first in the dispatch chain. Asserting an exclusion here would encode a
    # guarantee the code does not make. This assertion pins the actual gate so
    # that a future change adding batch-invariance awareness to QuickReduce
    # fails this test and forces the contract to be restated.
    qr_block = _extract(text, "self.qr_comm: QuickAllReduce", "QuickAllReduce(")
    assert "current_platform.is_rocm()" in qr_block
    assert "VLLM_BATCH_INVARIANT" not in qr_block
